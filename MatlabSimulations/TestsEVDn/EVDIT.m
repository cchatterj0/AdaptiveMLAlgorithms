...
for epoch = 1 : nEpochs
    for iter = 1 : nSamples
        ...
        W1 = W1 + etat1*(A*W1 - W1*(W1'*A*W1)) * inv(diag(diag(W1'*A*W1)));
      
        % Deflated Gradient Descent
        W2 = W2 + etat1*(A*W2 - W2*triu(W2'*A*W2)) * inv(diag(diag(W2'*A*W2)));
      
        % Weighted Gradient Descent
        W3 = W3 + etat2*(A*W3*C - W3*C*(W3'*A*W3)) * inv(diag(diag(W3'*A*W3)));
...
V1to4 = V(:,1:nEA)
%W4byNorm = W4*sqrt(inv(diag(diag(W4'*W4))))
%V1to4 = V(:,1:nEA)
